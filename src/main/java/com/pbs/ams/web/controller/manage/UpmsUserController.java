...
//        amsProductUser.setUserId(upmsUser.getUserId());
    ...
    @ResponseBody
    public Object update(@PathVariable("id") long id, UpmsUser upmsUser, Long productId, Long companyId, HttpServletRequest request) {
        ComplexResult result = FluentValidator.checkAll()
                .on(upmsUser.getUsername(), new LengthValidator(1, 20, "帐号"))
                .on(upmsUser.getRealname(), new NotNullValidator("姓名"))
                .doValidate()
        ...
        String oldName = request.getParameter("oldName");//用于判断修改的是否是用户名
        if (!oldName.equals(upmsUser.getUsername())) {//如果不相等说明修改的是用户名，做是否重复的判断
            UpmsUser user = upmsUserService.selectUserByUserName(upmsUser.getUsername());
            if (user != null) {
                return new ResultSet(StatusCode.REPETE_USERNAME, "用户名重复!");
            }
