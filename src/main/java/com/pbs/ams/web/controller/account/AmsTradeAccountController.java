...
public class AmsTradeAccountController extends BaseController {
    ...
    @Autowired
    private AmsProductUnitService amsProductUnitService;
    ...
    @Autowired
    private AmsStockOrderService amsStockOrderService;

    @Autowired
    private AmsStockOrderExecutionService amsStockOrderExecutionService;

    @Autowired
    private AmsProductUnitExtService amsProductUnitExtService;
    ...
    public Object list(
        ...
        List<Map> rows = amsProductUnitService.selectProductUnitWithDetail(params);
        long total = amsProductUnitService.selectProductUnitWithDetailCount(params);
    ...
    public Object delete(@PathVariable("ids") String ids) {
        if (StringUtils.isNotEmpty(ids)) {
            ...
            int count = amsProductUnitService.deleteByPrimaryKeys(idList);
            if (count > 0) {
    ...
    public Object create(AmsProductUnit amsProductUnit, Long productId) {
        if (null != amsProductUnit && null != productId) {
            AmsProductUnit account = amsProductUnitService.selectProductUnitByUnit(amsProductUnit.getTradeAccount());
            if (null != account) {
                return new ResultSet(StatusCode.REPETE_TRADE_ACCOUNT);
            }
        }
        Long tradeAccountId = IdGeneratorUtil.getKey("ams_trade_account");
        amsProductUnit.setProductUnitId(tradeAccountId);
        //构建账号产品关系实体
        amsProductPu amsProductPu = new amsProductPu();
        amsProductPu.setProductId(productId);
        amsProductPu.setProductUnitId(amsProductUnit.getProductUnitId());
        amsProductPu.setProductPuId(IdGeneratorUtil.getKey("ams_product_account"));
        int result = amsProductAccountService.insertSelective(amsProductPu);
        int count = 0;
        if (result > 0) {
            amsProductUnit.setOperatorId(getCurrentUser().getUserId());
            count = amsProductUnitService.insertSelective(amsProductUnit);
            return new ResultSet(StatusCode.ERROR_NONE, count);
        }
    ...
    public String update(@PathVariable("id") long id, HttpServletRequest request) {
        AmsProductUnit amsProductUnit = amsProductUnitService.selectByPrimaryKey(id);
        request.setAttribute("amsProductUnit", amsProductUnit);
    ...
    @ResponseBody
    public Object update(@PathVariable("id") Long id,AmsProductUnit amsProductUnit) {
        amsProductUnit.setProductUnitId(id);
        int count = amsProductUnitService.updateByPrimaryKeySelective(amsProductUnit);
        return new ResultSet(StatusCode.ERROR_NONE, count);
    }
    ...
    public Object updateAccountStatus(@PathVariable("id") Long id, String status) {
        ...
        AmsProductUnit amsProductUnit = new AmsProductUnit();
        amsProductUnit.setProductUnitId(id);
        int count = amsProductUnitService.updateAccountStatusById(id, Integer.parseInt(status));
        return new ResultSet(StatusCode.ERROR_NONE, count);
    }
    @Log(value = "账号详情")
    @RequiresPermissions("upms:account:read")
    ...
    public String details(@PathVariable("id") long id, HttpServletRequest request) {
        AmsProductUnit amsProductUnit = amsProductUnitService.selectByPrimaryKey(id);
        request.setAttribute("amsProductUnit", amsProductUnit);
        return "/account/query/query_tab.jsp";
    }
    ...
    public String details(HttpServletRequest request, @PathVariable("iframeName") String iframeName, @PathVariable("id") Long id){
        if (null != iframeName && null != id) {
            ...
            List<Map> amsProducts = amsProductService.selectProductWithDetail(params);
            List<Map> amsBrokers =amsBrokerService.selectBrokerWithDetail(params);
            amsProductPu amsProductPu =amsProductAccountService.getProductIdByAccountId(id);
            List<UpmsCompany> upmsCompany = upmsCompanyService.listCompanies(params);
            request.setAttribute("amsProducts",amsProducts);
            request.setAttribute("amsBrokers",amsBrokers);
            request.setAttribute("amsProductPu", amsProductPu);
            request.setAttribute("upmsCompany",upmsCompany);
            if ("update_account_base".equals(iframeName)){
                return "/account/basic/" + iframeName + ".jsp";
            }else if ("blackWhiteList".equals(iframeName)){
                return "/account/blackWhite/" + iframeName + ".jsp";
            }else if ("charge".equals(iframeName)){
                return "/account/charge/" + iframeName + ".jsp";
            }
            return "/account/query/" + iframeName + ".jsp";
        }
        return null;
    }
    @Log(value = "持仓列表")   //ams_stock_holding
    @RequiresPermissions("upms:account:read")
    ...
    public Object  positionList(
            @RequestParam(required = false, defaultValue = "0", value = "offset") int offset,
            @RequestParam(required = false, defaultValue = "10", value = "limit") int limit,
            @RequestParam(required = false, defaultValue = "", value = "positionDate") String positionDate) {
        Map<String, Object> params = Maps.newHashMap();
        if (StringUtils.isNotEmpty(positionDate)) {
            Long date = Long.parseLong(positionDate.replaceAll("-",""));
...
//    }
    ...
    public Object assetsList(
        ...
        if (StringUtils.isNotEmpty(dateStart)) {
            Long date = Long.parseLong(dateStart.replaceAll("-",""));
            params.put("dateStart",date);
        }
        if (StringUtils.isNotEmpty(dateEnd)) {
            Long date = Long.parseLong(dateEnd.replaceAll("-",""));
            params.put("dateEnd",date);
        }
        List<Map> rows = amsStockOrderService.selectAmsStockOrderWithDetail(params);
        long total = amsStockOrderService.selectAmsStockOrderWithDetailCount(params);
        Map<String, Object> result = new HashMap<String, Object>();
        result.put("total", total);
        result.put("rows", rows);
        return result;
    ...
    @Log(value = "成交列表")
    @RequiresPermissions("upms:account:read")
    @RequestMapping(value = "/amsTradeOrderFeed", method = RequestMethod.GET)
    @ResponseBody
    public Object AmsKnockList(
        ...
        params.put("offset", offset);
        params.put("limit", limit);
        List<Map> rows = amsStockOrderExecutionService.selectStockOrderExecutionWithDetail(params);
        long total = amsStockOrderExecutionService.selectStockOrderExecutionWithDetailCount(params);
        Map<String, Object> result = new HashMap<String, Object>();
        result.put("total", total);
        result.put("rows", rows);
        return result;
